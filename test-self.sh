...
mkdir -p ~/.ssh-sync-test

# Create master key
echo "testpassword123" > ~/.ssh-sync-test/master.key
chmod 600 ~/.ssh-sync-test/master.key

# Create client config
cat > ~/.ssh-sync-test/config.yaml << 'CLIENTCONFIG'
server:
...
backup:
  enabled: true
  dir: /home/ubuntu/.ssh-sync-test/backups
  max_backups: 10
encryption:
  master_key_file: /home/ubuntu/.ssh-sync-test/master.key
CLIENTCONFIG

echo -e "${GREEN}✓ Client initialized${NC}"
echo -e "${YELLOW}  Config: ~/.ssh-sync-test/config.yaml${NC}"
echo -e "${YELLOW}  Master key: ~/.ssh-sync-test/master.key${NC}"
...
echo "$TOKEN" > ~/.ssh-sync-test/token
chmod 600 ~/.ssh-sync-test/token
...
export SSH_SYNC_CONFIG=~/.ssh-sync-test/config.yaml

# Manually push via API for testing
MASTER_PASSWORD=$(cat ~/.ssh-sync-test/master.key)
...
echo -e "${YELLOW}Testing: ssh-sync list${NC}"
./cli/bin/ssh-sync list 2>&1 | head -5 || echo -e "${YELLOW}(Expected: requires login)${NC}"
